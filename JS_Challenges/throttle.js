// Source: https://www.algoexpert.io/frontend/coding-questions/throttle
...
function throttle(callback, delay) {
  ...
  function throttled(...args) {
    ...
    let timeSinceLastCall = currentTime - lastCalledTime;
    let delayRemaining = delay - timeSinceLastCall;

    if (delayRemaining <= 0) {
      // Case 1: Delay passed -> execute callback immediately
      lastCalledTime = currentTime;
      callback.apply(this, args);
    } else {
      // Case 2: Still within delay -> schedule trailing call
      clearTimeout(timerId);
      timerId = setTimeout(() => {
        lastCalledTime = Date.now(); // update the lastCalledTime
        callback.apply(this, args);
      }, delayRemaining);
    }
  }
